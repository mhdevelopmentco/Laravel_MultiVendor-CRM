...
class MerchantauctionController extends Controller {
	...
	*/
	...
	public function add_auction()
	{
			...
			{
    	 $merchantheader 		= view('sitemerchant.includes.merchant_header')->with('routemenu',"auction");	
		 $merchantleftmenus	= view('sitemerchant.includes.merchant_left_menu_auction');
		 ...
		 $shopdetails=Auction::get_shop_details();	
		 return view('sitemerchant.add_auction')->with('merchantheader', $merchantheader)->with('merchantleftmenus', $merchantleftmenus)->with('merchantfooter', $merchantfooter)->with('action','save')->with('category',$category)->with('merchant_details',$merchant_details)->with('shopdetails',$shopdetails);
			}
			else
			{
	...
	public function edit_auction($id)
	{
		...
		if (Session::has('merchantid'))	
		{
		 ...
		 $merchantfooter 		= view('sitemerchant.includes.merchant_footer');
		 $category  = Auction::get_category();
		 $auction_list  = Auction::get_merchant_auction($id,$merchantid);
		 $merchant_details  = Auction::get_merchant_details();
		 $shopdetails=Auction::get_shop_details();	 
		 
		 	if($auction_list)
			{
						 return view('sitemerchant.edit_auction')->with('merchantheader', $merchantheader)->with('merchantleftmenus', $merchantleftmenus)->with('merchantfooter', $merchantfooter)->with('action','update')->with('category',$category)->with('auction_list',$auction_list)->with('merchant_details',$merchant_details)->with('shopdetails',$shopdetails);
			}
			else
			{
		 return  Redirect::to('sitemerchant');
	...
	public function edit_auction_submit()
	{
		...
		$file_name_insert = $filename . "/**/".$filename_new_get;
		 ...
		 return Redirect::to('merchant_manage_auction')->with('block_message','Deal Updated Successfully');
	}
	...
	public function manage_auction()
	{
		if (Session::has('merchantid'))	
		{
		 ...
		 $merchantleftmenus	= view('sitemerchant.includes.merchant_left_menu_auction');
		 $merchantfooter 		= view('sitemerchant.includes.merchant_footer');
		 $return 			= Auction::get_merchant_auction_details($date,$merchantid);
		 return view('sitemerchant.manage_auction')->with('merchantheader', $merchantheader)->with('merchantleftmenus', $merchantleftmenus)->with('merchantfooter', $merchantfooter)->with('auction_details',$return );
		}
		else 
		{
	...
	public function expired_auction()
	{
		if (Session::has('merchantid'))	
		{
		 ...
		 $merchantleftmenus	= view('sitemerchant.includes.merchant_left_menu_auction');
		 $merchantfooter 		= view('sitemerchant.includes.merchant_footer');
		 $return 			= Auction::get_merchant_expired_auction($date,$merchantid);
		 
		 return view('sitemerchant.expired_auction')->with('merchantheader', $merchantheader)->with('merchantleftmenus', $merchantleftmenus)->with('merchantfooter', $merchantfooter)->with('auction_details',$return );
	...
	public function block_auction($id,$status)
	{
		...
		if($status == 1)
		{
			return Redirect::to('merchant_manage_auction')->with('block_message','Deal Activated');
		}
		else if($status == 0)
		{
			return Redirect::to('merchant_manage_auction')->with('block_message','Deal Blocked');
		}
	...
	public function auction_winners()
	{
	   $merchantid=Session::get('merchantid');
		 ...
		 $merchantleftmenus	= view('sitemerchant.includes.merchant_left_menu_auction');
		 $merchantfooter 		= view('sitemerchant.includes.merchant_footer');
		 $getauctionidlistrs=Merchant::getauctionidlist($merchantid);
		  $getauctionidlist= $getauctionidlistrs[0]->proid;
		 $action_winners   = Merchant::get_auction_winners($getauctionidlist);
		 
		 return view('sitemerchant.auction_winners')->with('merchantheader', $merchantheader)->with('merchantleftmenus', $merchantleftmenus)->with('merchantfooter', $merchantfooter)->with('auctiondetails',$action_winners);
	...
	public function auction_cod()
	{
		...
		$merchantheader 		= view('sitemerchant.includes.merchant_header')->with('routemenu',"auction");	
		 ...
		 $merchantfooter 		= view('sitemerchant.includes.merchant_footer');
		  $getauctionidlistrs=Merchant::getauctionidlist($merchantid);
		  $getauctionidlist= $getauctionidlistrs[0]->proid;
		 $action_winners   =Merchant::get_auction_cod($getauctionidlist); 
	...
	public function auction_details($id)
	{
		if (Session::has('merchantid'))	
		{
		 ...
		 $merchantleftmenus	= view('sitemerchant.includes.merchant_left_menu_auction');
		 $merchantfooter 		= view('sitemerchant.includes.merchant_footer');
		 $return 			= Auction::get_merchant_auction_view($id,$merchantid);
		 if($return)
		 {
	 return view('sitemerchant.auction_details')->with('merchantheader', $merchantheader)->with('merchantleftmenus', $merchantleftmenus)->with('merchantfooter', $merchantfooter)->with('auction_list',$return );
		 }
		 else
		 	{
				return Redirect::to('sitemerchant');
