    ...
    <div id="wrap">
       ...
       {!! $adminleftmenus !!}
        ...
        <div id="content">
				...
				<center>
		 <form  action="{!!action('DealsController@manage_deals')!!}" method="POST">
...
if(isset($_POST['submit']))
 ...
 foreach($dealrep as $row) { 
									  $deal_get_img = explode("/**/",$row->deal_image);
									  if($row->deal_status == 1)
									  {
										  $process = "<a href='".url('block_deals/'.$row->deal_id.'/0')."' > <i style='margin-left:10px;' class='icon icon-ok icon-me'></i> </a>";
								  } else if($row->deal_status == 0) {
										   $process = "<a href='".url('block_deals/'.$row->deal_id.'/1')."' > <i style='margin-left:10px;' class='icon icon-ban-circle icon-me'></i> </a>";
									  }
									  ?>
                                    <tr class="gradeA odd">
                                            <td class="sorting_1"><?php echo $i; ?></td>
                                            <td class="center  "><?php echo substr($row->deal_title,0,45); ?></td>
                                            <td class="center  "><?php echo $row->stor_name; ?></td>
                                            <td class="center  "><?php echo $row->deal_original_price; ?></td>
                                            <td class="center  "><?php echo $row->deal_discount_price; ?></td>
                                            <td class="center  "><a ><img style="height:40px;" src="<?php echo url(''); ?>/assets/deals/<?php echo $deal_get_img[0]; ?>"></a></td>
                                           
                                            <td class="center  ">
                                           <a href="<?php echo url('edit_deals/'.$row->deal_id); ?>" > <i class="icon icon-edit" style="margin-left:15px;"></i></a>
                                         	<?php echo  $process; ?>
                                             <?php if(count($delete_deals) == "") { ?><a href="<?php echo url('delete_deals/'.$row->deal_id); ?>" > <i class="icon icon-trash icon-1x" style="margin-left:15px;"></i></a> <?php } else { foreach($delete_deals as $dd) { } if($dd->order_pro_id != $row->deal_id) { ?>  <a href="<?php echo url('delete_deals/'.$row->deal_id); ?>" > <i class="icon icon-trash icon-1x" style="margin-left:15px;"></i></a><?php  } } ?>
                                            </td>
                                            <td class="center  "><a href="<?php echo url('dealview'); ?>/<?php echo $row->deal_id; ?>" target="_blank"><i class="icon-search icon-2x"></i></a></td>
                                            <td class="center  "><a href="<?php echo url('deal_details')."/".$row->deal_id; ?>">View details</a></td>
                                        </tr>
									<?php $i++; }
			...
			else
			{
									  ...
									  {
										  $process = "<a href='".url('block_deals/'.$deal_list->deal_id.'/0')."' > <i style='margin-left:10px;' class='icon icon-ok icon-me'></i> </a>";
								  } else if($deal_list->deal_status == 0) {
										   $process = "<a href='".url('block_deals/'.$deal_list->deal_id.'/1')."' > <i style='margin-left:10px;' class='icon icon-ban-circle icon-me'></i> </a>";
									  }
									  ?>
                                    <tr class="gradeA odd">
                                            <td class="sorting_1"><?php echo $i; ?></td>
                                            ...
                                            <td class="center  ">
                                           <a href="<?php echo url('edit_deals/'.$deal_list->deal_id); ?>" > <i class="icon icon-edit" style="margin-left:15px;"></i></a>
                                         	<?php echo  $process; ?>
                                            ...
                                            <td class="center  "><a href="<?php echo url('deal_details')."/".$deal_list->deal_id; ?>">View details</a></td>
                                        </tr>
									<?php $i++; }
			}									?>
